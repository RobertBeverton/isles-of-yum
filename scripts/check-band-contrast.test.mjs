...
describe("checkBandContrast", () => {
  ...
  it("flags a color that fails contrast at a given mix percentage", () => {
    // #fdf6ec is near-white and passes comfortably at the real 35% band mix,
    // so this exercises the mixPercent parameter directly at a heavier mix
    ...
    expect(checkBandContrast(paleColor, 0.65).length).toBeGreaterThan(0);
  });

  it("the committed BAND_MIX_PERCENT matches what style.css's .rail-group-content rule actually uses", () => {
    ...
    expect(BAND_MIX_PERCENT).toBe(0.35);
  });
});
